...
describe('Fetch products by name (E2E)', () => {
  ...
  it('[GET] /search/products?name=example&page=2 → It must be able to fetch products by name', async () => {
    for (let i = 0; i < 22; i++) {
    ...
    const response = await request(app.getHttpServer())
      .get('/search/products?name=example&page=2')
      .send();
    expect(response.statusCode).toBe(200);
    expect(response.body.products).toHaveLength(2);
    expect(response.body).toEqual({
      products: expect.arrayContaining([
        expect.objectContaining({ name: 'product example 20' }),
        expect.objectContaining({ name: 'product example 21' }),
      ]),
    });
  });
});
